...
export class TimelinechartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.appSettingsService.getJSON().subscribe(data => {
      ...
      const averageSpeed = data.result.segmentSpeeds.map(res => res.segments.map(result => result.travelTimeMinutes));
      const segmentCode = data.result.segmentSpeeds.map(res => res.segments.map(x => x.code));
